...
前一轮完整命令通过了135项测试；本轮新增结构分片 harness 后，回归总数为136项，0失败/错误/跳过。Debug和测试APK构建成功，Lint无错误。本地原图探针在独立运行验证后移出源码目录，未靠跳过正式测试获得通过。
...
最终主APK SHA-256为`30F0375CF94E0A7F0492DD205FA68187CD0B112B575A47044E5227EAA4F94274`，与设备`base.apk`一致；后续Gradle修改仅更换等价测试assets目录声明，未改变主APK。136项本机测试全部通过，真实原图独立模型测试报告`OK (1 test)`，二者证据分别记录。
