...
2. 点击“开始整理 / 继续未完成页面”。支持多级表头、矩形合并区域、不同列宽、30+列、局部断线和有限方向/透视变化。存在歧义时先进入“确认 / 调整表格结构”，核对原图覆盖，按百分比调整区域、行列分割、表头和合并，保存草稿或确认后继续。同任务后页优先复用已确认模板，明显异表和新的结构歧义仍需核对。
...
相机、任意角度/重度透视恢复、无边框表、自由拖拽图片编辑器、模糊重复匹配和跨任务模板库不在当前实现中。
结构编辑采用最小数值输入方式，尚需手机操作验收；数据区合并只在左上格保留候选内容，其余覆盖格不猜值，相关值需复核。
...
Room v6 保存任务、来源、动态结构、页处理结果、问题、人工决定和页面结构草稿/确认；包含 v1→v2→v3→v4→v5→v6 完整迁移。
